...
let GRADLE_VERSION = process.env.GRADLE_VERSION;
if (GRADLE_VERSION) {
  let major = parseInt(GRADLE_VERSION.split(".")[0]);
  if (major < 5) {
...
if (kotlinSupported) {
  test('build.gradle.kts files are supported', {timeout: 150000}, async (t) => {
