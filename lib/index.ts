...
// Both script names appear as string literals in every path.join below, rather
// than being passed through as a variable. "pkg" detects assets by statically
// analysing these calls, so a computed filename would leave the script out of the
// standalone Snyk CLI binary. Both are also listed under "files" in package.json
// so they reach the npm tarball.
function initScriptAssetPath(scriptName: InitScriptName): string {
  if (/index.js$/.test(__filename)) {
    ...
    return scriptName === 'init-cc.gradle'
      ? path.join(__dirname, '../lib', 'init-cc.gradle')
      : path.join(__dirname, '../lib', 'init.gradle');
  }
  if (/index.ts$/.test(__filename)) {
    // running from ./lib
    return scriptName === 'init-cc.gradle'
      ? path.join(__dirname, 'init-cc.gradle')
      : path.join(__dirname, 'init.gradle');
  }
  throw new Error(`Cannot locate Snyk ${scriptName} script`);
}

async function injectedPlugin(scriptName: InitScriptName): Promise<{
  injectedPluginFilePath: string;
  cleanupCallback?: () => void;
}> {
  const initGradleAsset: string = initScriptAssetPath(scriptName);
...
type InitScriptName = 'init.gradle' | 'init-cc.gradle';
...
// An unrecognised version answers false, so an unreadable `gradle -v` keeps the
// previous behaviour rather than opting a build into the newer script.
function supportsConfigurationCache(gradleVersionOutput: string): boolean {
  const version = parseGradleVersion(gradleVersionOutput);
  if (!version) {
    return false;
  }
  return (
...
function initScriptFor(gradleVersionOutput: string): InitScriptName {
  return supportsConfigurationCache(gradleVersionOutput)
    ? 'init-cc.gradle'
    : 'init.gradle';
}
...
async function getAllDeps(
  ...
  // Anchored on the dot so this matches only the 1.x line. Without it, "Gradle 10"
  // and every later two-digit major would also be rejected as unsupported.
...
function buildArgs(
  ...
  // init.gradle emits from a task action that walks every project, so parallel
  // builds race it and can produce multiple JSONDEPS lines. init-cc.gradle cannot:
  // each project reports itself into a shared build service holding thread-safe
  // state, and one close() does the printing, so exactly one line is emitted no
  // matter how many projects report concurrently.
  //
  // Under Isolated Projects the flag was never protecting the phase that matters
  // anyway: project configuration is parallel there regardless of this setting,
  // and configuration is where the injected script does its wiring.
  //
  ...
  // nor forced off.
  //
  // Forcing it off is not a neutral act on a modern build: Isolated Projects is
  // built on the configuration cache and cannot run without one, so Gradle
  // rejects the combination outright with "Configuration Cache cannot be disabled
  // when Isolated Projects is enabled" and the scan dies before resolving
